...
class ChatController:
    ...
    async def handle_prompt(self, bot_id: str, conversation_id: str, prompt: str, request: Request, response: Response):
        bot = await self.chat_repo.get_bot(bot_id=bot_id)
        if not bot:
            raise HTTPException(404, "Bot not found")
        
        conversation = await self.chat_repo.get_or_create_conversation(bot_id=bot_id, conversation_id=conversation_id, request=request, response=response)
        if not conversation:
